...
/**
  ...
  */
public
Getopt(final String progname, final String[] argv, final String optstring)
{
...
/**
  ...
  */
public
Getopt(final String progname, final String[] argv, String optstring,
...
/**
  ...
  */
protected void
exchange(final String[] argsv)
{
  int bottom = first_nonopt;
  final int middle = last_nonopt;
  int top = optind;
  String tem;
      ...
      if (top - middle > middle - bottom)
        {
          ...
          for (i = 0; i < len; i++)
            {
              tem = argsv[bottom + i];
              argsv[bottom + i] = argsv[top - (middle - bottom) + i];
              argsv[top - (middle - bottom) + i] = tem;
            }
  ...
  if (ambig && !exact)
    {
      if (opterr)
        {
          final Object[] msgArgs = { progname, argv[optind] };
              ...
              if (opterr)
                {
                  ...
                  if (argv[optind - 1].startsWith("--"))
                    {
                      final Object[] msgArgs = { progname, pfound.name };
                      System.err.println(MessageFormat.format(
                  ...
                  // +option or -option
                    ...
                    {
                      final Object[] msgArgs = { progname, new
      ...
      else if (pfound.has_arg == LongOpt.REQUIRED_ARGUMENT)
        {
          if (optind < argv.length)
          ...
          else
            {
              if (opterr)
                {
                  final Object[] msgArgs = { progname, argv[optind-1] };
                  System.err.println(MessageFormat.format(
          ...
          if (opterr)
            {
              if (argv[optind].startsWith("--"))
                {
                  final Object[] msgArgs = { progname, nextchar };
                  System.err.println(MessageFormat.format(
